...
using InventoryControlAngularJS.Migrations.SeedData;
...
namespace InventoryControlAngularJS.Tests
{
    public abstract class InventoryControlAngularJSTestBase : AbpIntegratedTestBase<InventoryControlAngularJSTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected InventoryControlAngularJSTestBase()
        {
        ...
        protected void UsingDbContext(Action<InventoryControlAngularJSDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<InventoryControlAngularJSDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<InventoryControlAngularJSDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<InventoryControlAngularJSDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<InventoryControlAngularJSDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<InventoryControlAngularJSDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<InventoryControlAngularJSDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<InventoryControlAngularJSDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<InventoryControlAngularJSDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<InventoryControlAngularJSDbContext, Task<T>> func)
        {
            T result;
